...
function rankTemplatesByCosine(templates, user, topN = 6) {
  ...
  if (!user || !user.objetivos || !user.metadados) {
    console.warn('Perfil do usuário incompleto, retornando templates sem ordenação');
    return templates.slice(0, topN);
  }

  // 1. Extrai perfil de usuário com verificação de segurança
  let freqMin = 3, freqMax = 5; // valores padrão
  try {
  ...
  templateDicts.concat(userDict).forEach(d =>
    Object.keys(d).forEach(f => featureSet.add(f))
  );
...
async function loadAllTemplates() {
  const resp = await fetch('../scripts/workouts/structured_workout_templates_final.json');
  return await resp.json();
}

function getProfileFilters(profile) {
  if (!profile || !profile.objetivos) return {};
  const { tipo_treino, objetivo_principal, experiencia_previa } = profile.objetivos;
  return {
...
function renderTemplateMenu(onSelect, templates, recommended) {
  ...
  function renderList(list) {
    ...
    list.forEach((tpl, idx) => {
      ...
      btn.className = 'btn btn-outline-primary d-flex align-items-center gap-2 animate__animated animate__fadeIn position-relative';
...
function criarNovoTreino() {
  ...
  const novoId = gymApp.edited_trainings.length > 0
...
function renderTreinoForm(treino) {
    let html = `
        ...
        <div class="mb-4">
            ...
            <select id="selecao-nivel" class="form-select mb-2">
                ...
                <option value="Ficha iniciante" ${treino.type == "Ficha iniciante" ? "selected" : ""}>Iniciante</option>
                <option value="Ficha intermediária" ${treino.type == "Ficha intermediária" ? "selected" : ""}>Intermediário</option>
                <option value="Ficha avançada" ${treino.type == "Ficha avançada" ? "selected" : ""}>Avançado</option>
            </select>
        </div>
        <div class="row g-4">
    `;
    ...
    document.querySelectorAll('.adicionar-exercicio').forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            treino.days[diaIdx].day.push({ exercise: "Novo exercício", series: "", repetitions: "" });
    ...
    document.querySelectorAll('.delete-ex').forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            if (confirm(`Confirmar exclusão de "${treino.days[diaIdx].day[exIdx].exercise}"?`)) {
    ...
    document.getElementById('adicionar-dia').addEventListener('click', e => {
        e.preventDefault();
        const novoId = treino.days.length > 0 ? Math.max(...treino.days.map(d => d.id)) + 1 : 1;
        treino.days.push({ id: novoId, xp: 100, name: `Dia ${novoId} - Nova ficha`, day: [] });
        renderTreinoForm(treino);
    });
    lucide.createIcons();
}
...
function atualizarTreinoComInputs(treino) {
    ...
    treino.days.forEach((dia, diaIdx) => {
        ...
        dia.day.forEach((ex, exIdx) => {
            ex.exercise = document.getElementById(`dia-${diaIdx}-nome-exercicio-${exIdx}`).value;
            ex.series = document.getElementById(`dia-${diaIdx}-series-exercicio-${exIdx}`).value;
            ex.repetitions = document.getElementById(`dia-${diaIdx}-rep-exercicio-${exIdx}`).value;
        });
    });
}
...
async function criarFicha() {
  ...
  if (!profile) {
    console.warn('Perfil do usuário não encontrado, usando templates sem filtro');
    // Renderiza menu sem recomendações personalizadas
    renderTemplateMenu((tpl) => {
    ...
    }, allTemplates, allTemplates.slice(0, 6));
  } else {
...
function renderizarGridTreinos() {
    ...
    gymApp.edited_trainings.forEach((treino, index) => {
        const totalExercicios = treino.days.reduce((total, dia) => total + dia.day.length, 0);
                ...
                <div class="treino-card" data-treino-index="${index}">
                    ...
                    <div class="treino-card-header">
                        <div>
                            ...
                            <div class="treino-card-category">${treino.category || 'Sem categoria'}</div>
                        </div>
                        <span class="treino-card-type">${treino.type || 'Geral'}</span>
                    </div>
...
function selecionarTreino(index) {
    if (!gymApp || !gymApp.edited_trainings || index < 0 || index >= gymApp.edited_trainings.length) {
        return;
    }
...
window.excluirTreino = function(index) {
    ...
    if (confirm(`Confirmar exclusão da ficha "${treino.name}"? Essa ação é irreversível.`)) {
