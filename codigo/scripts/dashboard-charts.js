...
async function renderVolumeCard(maquina = "flexora") {
  ...
  const gradient = ctxVolume.createLinearGradient(
    0,
    0,
    0,
    ctxVolume.canvas.clientHeight
  );
        ...
        tooltip: {
          ...
          callbacks: {
            label: function (context) {
              ...
              if (
                context.dataIndex === 2 &&
                context.dataset.data[context.dataIndex] === 150
              ) {
...
async function renderCargaCard(maquina = "flexora") {
  const ctxCarga = document
    .getElementById("progressaoCargaChart")
    ?.getContext("2d");
